...
<div class="content">
    <div class="block block-rounded">
        <div class="block-content">
            <form action="{{route('sr.to.br.transfer.confirm')}}" method="post" id="form_1">
                     @csrf
                <div class="p-3 px-10" id="select_branch_row">
                <div class="row mb-3 shadow rounded border">
                <div class="col-md-12 p-2">
                        <div class="p-1">
                            <label for=""><span class="text-danger">*</span>Sender SR</label>
                    ...
                    <div class="col-md-12">
                        <div class="p-1 text-right">
                            <button type="button" onclick="branch_and_sr_select_next_step()" class="btn btn-dual text-light mb-2 bg-primary mr-2">Next</button>
                        </div>
                    </div>
                </div>
                </div>
...
function branch_and_sr_select_next_step() {
    var sender_branch = $('#sender_branch').val();
    var sr = $('#sr').val();
    if(sender_branch != 0 && sr != 0) {
        var sender_branch_name = $("#sender_branch option:selected" ).text();
        var receiver_sr_name = $("#sr option:selected" ).text();

        $('#main_body_row').show();
        $('#select_branch_row').hide();
        $('#sender_branch_title_for_product').text(receiver_sr_name);
        var title = receiver_sr_name+" To <sapn class='text-info'>"+sender_branch_name+"</sapn> Transfer Product";
        $('#branch_titles').html(title);
    }
    else {
        error('Please Select Branch & SR!!!');
    }
}
...
$('#product_title').keyup(function(){
    ...
    $.ajax({
        type : 'get',
        url: '/admin/products/sr-to-branch-transfer/search_products',
        data:{
            'title':title,
            'sr':sr,
        },
