...
<div class="content">
    <div class="block block-rounded">
        <div class="block-content">
            <form action="{{route('customer.direct.return.products')}}" method="post" id="form_1">
            @csrf
            <div class="p-3 px-8" id="select_branch_row">
            <div class="row mb-3 shadow rounded border">
                    ...
                    <div class="p-1">
                        <div class="card bg-white border table-contentpos mb-3" style="background-color: #fff !important;">
							<div class="card-body row" style="padding: 9px !important;">
							    <div class="col-md-12">
                                    <div class="row" style="" id="search_customer_shown">
                                        <div class="col-md-6">
								    ...
								    <div class="justify-content-between colorfull-select row" id="customer_info_shown" style="display: none;">
								        <div class="col-md-12">
                ...
                <div class="col-md-12 p-2">
                    <div class="p-1">
                        <label for=""><span class="text-danger">*</span>SR</label>
                        <select name="sr" id="sr" class="form-control">
                            <option value="0">-- Select SR --</option>
                            @foreach ($all_sr as $sr)
                            <option value="{{$sr->id}}">{{$sr->name}} [ {{$sr->area_info->name}} ]</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                <div class="col-md-12">
                    <div class="p-1 text-right">
                        <button type="button" onclick="customer_and_sr_select_next_step()" class="btn btn-dual text-light mb-2 bg-primary mr-2">Next</button>
                    </div>
                </div>
            </div>
            </div>
            <div class="row" id="main_body_row" style="display: none;">
                <div class="col-md-3">
                <h6><b>Search Products From <span id="sender_branch_title_for_product"></span></b></h6>
                    <div class="" >
                ...
                <input type="hidden" name="" id="toggle_yes" value='1'>
                    <div class="" style="padding: 0px 2px;">
                        <h4 class="fw-bold mb-2" id="main_title"></h4>
                        <div id="customer_info_main"></div>
...
function myFunction(row_id, id, variation_id, variation_name, purchase_line_id, lot_number, name, purchase_price, sales_price, vat, discount, discount_amount, stock, unit_type, is_cartoon, cartoon_quantity, cartoon_amount, invoice_number) {
     ...
     if($('#check_id'+generate_id).val()) {
        ...
        const cartDom = `<tr id="cart_tr`+generate_id+`">
                            <td>
                                ...
                                <small><b>Invoice Number: #</b> `+invoice_number+`</small>
                                <select class="form-select" required name="return_or_exchange[]">
                            ...
                            <td>
                                <div class="mb-2 p-1 border rounded shadow">
                                    ...
                                    <br><small>Max `+stock+` `+unit_type+`</small>
                                </div>
                            </td>
                            <td>
                                <div class="mb-2 p-1 border rounded shadow">
...
function customer_and_sr_select_next_step() {
    var customer_id = $('#customer_id').val();
    var sr = $('#sr').val();
    if(customer_id != 0 && sr != 0) {
        var sr_name = $("#sr option:selected" ).text();

        $('#main_body_row').show();
        $('#select_branch_row').hide();
        //$('#sender_branch_title_for_product').text(sr_name);
        var title = "Product Return To Customer from <sapn class='text-info'>"+sr_name+"</sapn>";
        $('#main_title').html(title);
    }
    else {
        error('Please Select Customer & SR!!!');
    }
}
...
$('#customer_search').keyup(function () {
        ...
        $.ajax({
            type: 'get',
            url: '/branch/return-products/search_customer',
            data: {
    ...
    function select_customer(id, name, phone, area) {
        const cartDom = `<div class="col-md-12 shadow rounded border mt-1 mb-1">
                            <label class="border-bottom">Customer Info</label><br>
                            <p class="font-weight-bold">`+name+` || `+phone+` || Area: `+area+`</p>
                        </div>`;

        $('#customer_show_info').html(cartDom);
        $('#customer_info_main').html(cartDom);
        $('#sender_branch_title_for_product').text(name+" [ "+phone+" ]");
...
$('#product_title').keyup(function(){
    ...
    $.ajax({
        type : 'get',
        url: '/branch/return_products/search_products',
        data:{
            'title':title,
